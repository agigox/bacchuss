...
const renderCustomizedLabel = (props, colors) => {
  ...
  return (
      <text x={x} y={y} fill={colors[index]} textAnchor={x > cx ? "start" : "end"} dominantBaseline="central" className="percent-pie">
...
`;
const BacPieChart = ({data, colors, miniSize}) => {

  return (
    <StyledRow className={`${miniSize ? 'pie-chart-row flex-column' : 'pie-chart-row'}`} minisize={miniSize}>
          ...
          <PieChart>
            ...
            >
              {data.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={colors[index]} strokeWidth={4} />
              ))}
            </Pie>
          </PieChart>
        </ResponsiveContainer>
